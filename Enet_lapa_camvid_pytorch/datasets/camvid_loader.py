...
#         [
#             transforms.Resize(size=(224, 224)),   # Resize：尺寸随意变大变小;人脸固定为224输入
#             transforms.RandomAffine(degrees=30),  # 中心旋转角度degrees
#             transforms.ColorJitter(brightness=0.5, contrast=0.5),
#             AddGaussianNoise(),
#             transforms.ToTensor(),  # 
#             transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]), # 后norma
#         ]
#     )


class CamvidPngPng(Dataset):
    def __init__(self, datapath, list_filename, training, crop_h, crop_w, channel):
        ...
        #  lapa-error:Floating point exception(core dumped)
